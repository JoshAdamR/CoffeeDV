        ...
        if cart_items:
            for cart_item in cart_items:
                ...
                if st.button(f"Complete Pickup for {cart_item['cart_id']}", key=f"complete_{cart_item['cart_id']}"):
                    ...
                    cart_ref.document(cart_item['cart_id']).update({'status': 'Collected', 'collected_date': datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
...
def get_next_feedback_id():
    try:
        ...
        if not cart.empty:
            ...
            last_order_id = last_order.get('order_id', None)
            # Return the order_id of the last row if it exists
            return last_order_id # safely get 'order_id' field
        else:
